...
// Store is the single point of access to the database.
// All SQL lives here — no SQL in handlers or the pipeline runner.
...
func (s *Store) CreateJob(job models.PipelineJob) error {
...
func (s *Store) GetJob(id string) (*models.PipelineJob, error) {
...
func (s *Store) ListJobs() ([]models.PipelineJob, error) {
	rows, err := s.db.Query(
...
func (s *Store) MarkFinished(id string, status models.JobStatus, recordCount, errorCount int) error {
	now := time.Now()
	_, err := s.db.Exec(
...
func (s *Store) SaveError(e models.ValidationError) error {
	_, err := s.db.Exec(
...
func (s *Store) GetErrors(jobID string) ([]models.ValidationError, error) {
	rows, err := s.db.Query(
...
func (s *Store) SaveAggregation(r models.AggregationResult) error {
...
func (s *Store) GetAggregation(jobID string) (*models.AggregationResult, error) {
	var data []byte
	err := s.db.QueryRow(
